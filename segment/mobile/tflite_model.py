...
converter.optimizations = [tf.lite.Optimize.OPTIMIZE_FOR_SIZE]  # 量化
...
# 预测结果
title = "Label: {}   prob: {:.3}".format(classes[class_index], probs_lite[class_index])
plt.title(title)
plt.show()  # 显示图片和预测结果
# 用测试集评估TFLite模型与转换前的原始模型
# 加载已经训练完成的鸟类识别模型
model_path = './saved_model/birds_model.h5'
model = tf.keras.models.load_model(model_path, custom_objects={'HardSwish': HardSwish,
